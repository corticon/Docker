...
# Set Corticon HOME and WORK dircectories
export CATALINA_OPTS="$CATALINA_OPTS -DCORTICON_HOME=/opt/corticon"
export CATALINA_OPTS="$CATALINA_OPTS -DCORTICON_WORK_DIR=/opt/corticon/work"
export CATALINA_OPTS="$CATALINA_OPTS -DCORTICON_LICENSE=/opt/corticon/work/CcLicense.jar"

# Create if Corticon CDD directory does not exist
mkdir -p /opt/corticon/work/cdd

#Create if Extensions directory does not exist
mkdir -p /opt/corticon/work/CorticonExtensions
mkdir -p /usr/local/tomcat/webapps/axis

cd /usr/local/tomcat/webapps/axis
unzip /opt/corticon/axis.war
cd .


CORTICON_EXTENSIONS="/opt/corticon/work/CorticonExtensions/*.jar"
echo $CORTICON_EXTENSIONS

# Copy the Extensions in axis/WEB-INF/lib
for file in $CORTICON_EXTENSIONS
do
	if [ -a "$file" ]
	then
		cp $file /usr/local/tomcat/webapps/axis/WEB-INF/lib
	fi
done
